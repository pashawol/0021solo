...
function eventHandler() {
	...
	$('.sUsage').each(function () {
		var usageSlider = new Swiper($(this).find('.usage-slider-js'), {
			...
			//nav
			navigation: {
				nextEl: '.usage-next--js',
				prevEl: '.usage-prev--js'
			}
		});
	}); //usageSlider
	...
	$('.sCatalog').each(function () {
		var catalogSlider = new Swiper($(this).find('.catalog-slider-js'), {
			...
			//nav
			navigation: {
				nextEl: '.catalog-next--js',
				prevEl: '.catalog-prev--js'
			}
		});
	}); //
	...
	var cleanSlider = new Swiper('.clean-slider-js', {
		...
		//nav
		navigation: {
			nextEl: '.clean-next--js',
			prevEl: '.clean-prev--js'
		}
	}); //promo slider
	...
	var promoSlider = new Swiper('.promo-slider-js', {
		...
		//nav
		navigation: {
			nextEl: '.promo-next--js',
			prevEl: '.promo-prev--js'
		}
	}); //newsSlider
	...
	window.addEventListener('scroll', function () {
		if (window.matchMedia("(min-width: 1024px)").matches) {// closeAllMenu();
		}
	}, {
		passive: true
	});
	...
	$('img.img-svg-js, .menu-image, .sBalls__icon img, .sFolow__item img, .sIcons__item img, .sUsage__icon img').each(function () {
		...
		$.get(imgURL, function (data) {
			...
			$svg = $svg.removeAttr('xmlns:a').style.removeProperty('enable-background'); // Check if the viewport is set, if the viewport is not set the SVG wont't scale.
